...
function buildCandidateUrls(locale: Locale) {
  ...
  const url = new URL(`${apiUrl}/v2/projects/${projectId}/translations/${locale}`);
...
function tryExtractFromEmbeddedKeys(payload: Record<string, unknown>, locale: Locale) {
...
function extractMessagesFromPayload(payload: unknown, locale: Locale): IntlMessages {
...
async function fetchTolgeeMessages(locale: Locale): Promise<IntlMessages> {
  const urls = buildCandidateUrls(locale);
  const errors: string[] = [];
  ...
  throw new Error(
    `Unable to load Tolgee messages for locale "${locale}". ${errors.join("; ")}`,
  );
}
